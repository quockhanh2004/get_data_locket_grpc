import { ListenResponse, TimestampValue } from "../models/firebase.model";
import { Post } from "../models/posts.model";

function simplifyFirestoreData(data: ListenResponse): Post | null {
  if (!data.document_change?.document?.fields) {
    return null;
  }
  const fields = data.document_change.document.fields;
  const timestampToFloat = ({ seconds, nanos }: TimestampValue) =>
    parseInt((Number(seconds) + Number(nanos) / 1e9).toFixed(0));
  ...
  return {
    ...
    create_time: timestampToFloat(data.document_change.document.create_time),
    update_time: timestampToFloat(data.document_change.document.update_time),
